...
class MagnitParser:
    ...
    def get_product(self, product_soup):            #Основные изменения здесь проведены
        dt_parser = self.date_parse(product_soup.find('div', attrs={'class': 'card-sale__date'}).text)

        product_template = {                    #работа через шаблон
